...
class TaastrupgaardNetwork(MobilityNetworkBase):
    def __init__(self):
        super().__init__(name="taastrupgaard")
        # create nodes 
        self.inner_node = self.node_30(node_id="inner", gps=(55.653992308213944,12.289811420268117), radius=0.75, filter_gps_zones=[])
        self.frontier_node = self.node_30(node_id="frontier", gps=(55.653992308213944,12.289811420268117), radius=1.5, filter_gps_zones=[])
        ...
        self.city_commute = self.connection_train(conn_id="commute", origin_node=self.inner_node, origin_mobility=self.connection_walk(), 
                                            dest_node=self.cph_central_node, dest_mobility=self.connection_walk(), train_line=trains.all.value, 
                                            depart_time=mp.TIMING(year=2025, month=4, day=1, hour=9, minute=0))
        
        # QUERY / READ
        self.READ()
        #self.visualize_trips(sample_id="walk", res=3, type='time', gps_0=(55.63075689433673, 12.228602446242164), gps_1=(55.67279195176489, 12.317008052455543), heatmap_id="taastrupgaard_walk_time", aggregate_type="mean", on_cph_land=False)
        #self.visualize_trips(sample_id="bike", res=3, type='time', gps_0=(55.63075689433673, 12.228602446242164), gps_1=(55.67279195176489, 12.317008052455543), heatmap_id="taastrupgaard_bike_time", aggregate_type="mean", on_cph_land=False)
